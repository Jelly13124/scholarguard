# The ScholarGuard — Week 1 Foundation Implementation Plan
...
├── voice/
│   └── the-scholarguard-voice.md           CREATE — output of voice-builder skill
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "chore: add python venv + requirements"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "chore: scaffold project directory layout"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: add buffer config loader with env validation"
```
...
Only the operations needed for The ScholarGuard content pipeline are
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: buffer client with createPost and deletePost (draft mode)"
```
...
```powershell
git add scripts/buffer_client.py tests/test_buffer_client.py
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: x thread support via threadedPosts metadata"
```
...
```powershell
git add .env.example
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "docs: add .env.example reference"
```
...
- [ ] **Step 3: Purchase thescholarguard.com**

Recommended registrar: **Cloudflare Registrar** (https://dash.cloudflare.com/) — at-cost pricing, no upselling, free WHOIS privacy.

Sign in, search `thescholarguard.com`. Approx $9-12/year. If taken, fall back to `thescholarguard.co` or `appeallab.io` (update spec accordingly).

Verification:
```powershell
nslookup thescholarguard.com
```
...
git add .env.example
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "docs: add convertkit and gemini placeholders to env example"
```
...
Go to https://twitter.com/settings/screen_name. Change `RuizheYuan15453` → `thescholarguard`. Save.

Verification: visit https://twitter.com/thescholarguard. Expected: your profile loads.
...
Expected: in the response, the twitter channel `name` field is `thescholarguard`.
...
Go to https://publish.buffer.com → click on the X channel (now @thescholarguard). Click **Settings** (gear icon) → **Posting Schedule** → **Time Zone** dropdown → select `(GMT-05:00) New York` → **Save**.

- [ ] **Step 2: Change IG channel timezone to America/New_York**

Repeat Step 1 for the Instagram channel (@thescholarguard).
...
## Task 10: Generate The ScholarGuard Voice Doc

**Files:**
- Create: `voice/the-scholarguard-voice.md`
...
- [ ] **Step 2: Invoke voice-builder with The ScholarGuard inputs**

Tell Claude:
> "Invoke the voice-builder skill to create a voice doc for 'The ScholarGuard'. Inputs:
>
> - **Brand:** The ScholarGuard — faceless case-analysis research team. Tagline: 'We break down real US & UK academic cases — appeals, study, and what actually works.'
...
> - **Save to:** `voice/the-scholarguard-voice.md`"

- [ ] **Step 3: Verify the voice doc covers required constraints**

Open `voice/the-scholarguard-voice.md` and confirm it contains all of:
...
```powershell
git add voice/the-scholarguard-voice.md
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: generate the appeal lab voice doc"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: case anonymization template with c-s-o structure"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: anonymize first 3 cases (P2 ai-detection, P1 misconduct, P3/P4)"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: anonymize cases 004-006"
```
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "feat: anonymize cases 007-010, complete 10-case foundation library"
```
...
- [x] X renamed @RuizheYuan15453 → @thescholarguard
...
- [x] thescholarguard.com purchased (or fallback recorded: ___)
- [x] Gemini API key in .env, verified working
- [x] voice/the-scholarguard-voice.md exists and covers required constraints
...
git -c user.name="The ScholarGuard" -c user.email="noreply@thescholarguard.com" commit -m "docs: w1 foundation complete status note"
```
